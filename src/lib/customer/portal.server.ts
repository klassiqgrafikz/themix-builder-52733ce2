...
/** Bank-scoped account number — always 12 digits, displayed as XXXX XXXX XXXX. */
export function generateAccountNumber(_countryCode?: string | null): string {
  const bytes = new Uint8Array(12);
  crypto.getRandomValues(bytes);
  let s = "";
  for (let i = 0; i < 12; i++) s += (bytes[i] % 10).toString();
  return s;
}
