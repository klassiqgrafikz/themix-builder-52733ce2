...
function PortalShellInner({
  ...
  const portalLayoutKey: PortalLayoutKey =
    ((manifest as unknown as { portal_layout_key?: PortalLayoutKey }).portal_layout_key) ??
    (variant === "corporate" ? "topnav" :
  ...
  switch (portalLayoutKey) {
    ...
    case "topnav":
      return (
        <div
          ...
          className="flex w-full flex-col" data-layout="topnav"
        >
    ...
    case "premium":
      return (
        <div
          ...
          className="flex w-full" data-layout="premium"
        >
    ...
    case "minimal":
      return (
        ...
        >
          <header className="fixed inset-x-0 top-0 z-20 flex items-center justify-between border-b bg-white px-4 py-3 shadow-sm">
            <div className="flex items-center gap-2">
              ...
              <Link to="/$slug/portal" params={{ slug }} className="flex items-center gap-2">
                {manifest.brand.dashboard_logo_url ? (
                  <img src={manifest.brand.dashboard_logo_url} alt="" className="h-7 w-7 object-contain" />
                ) : (
                  <span className="flex h-7 w-7 items-center justify-center rounded text-xs font-bold text-white" style={{ backgroundColor: primary }}>
                    {manifest.bank.name.slice(0, 1)}
                  </span>
                )}
                <span className="text-sm font-semibold text-slate-900">{manifest.bank.name}</span>
              </Link>
            </div>
            <div className="flex items-center gap-2">
              <div className="hidden text-xs text-slate-500 md:block">{customer.first_name} {customer.last_name}</div>
              <NotificationBell slug={slug} tone="light" />
            </div>
          </header>
          ...
          {drawerOpen && (
            <div className="fixed inset-0 z-40 md:hidden" onClick={() => setDrawerOpen(false)}>
              ...
              <div className="absolute inset-y-0 left-0 w-64 bg-white p-4 shadow-xl" onClick={(e) => e.stopPropagation()}>
                <div className="mb-4 flex items-center justify-between">
                  <span className="text-sm font-semibold text-slate-900">{manifest.bank.name}</span>
                  <button onClick={() => setDrawerOpen(false)} className="rounded p-1 text-slate-400 hover:bg-slate-100">
                    <X className="h-4 w-4" />
                  </button>
                </div>
                {nav.map((n) => {
          ...
          <main className="mx-auto w-full max-w-[1200px] flex-1 px-4 py-6 pt-20 md:px-8">{content}</main>
        </div>
      );
    ...
    case "floating":
      return (
        <div
          ...
          className="flex w-full flex-col"
        >
          <header className="fixed inset-x-0 top-0 z-20 flex items-center justify-between border-b bg-white px-4 py-3">
            <Link to="/$slug/portal" params={{ slug }} className="flex items-center gap-2">
              {manifest.brand.dashboard_logo_url ? (
                <img src={manifest.brand.dashboard_logo_url} alt="" className="h-7 w-7 object-contain" />
              ) : (
            ...
            <div className="flex items-center gap-2">
              <NotificationBell slug={slug} tone="light" />
              <button className="rounded-lg p-2 text-slate-500 hover:bg-slate-100 md:hidden" onClick={() => setDrawerOpen(true)} aria-label={t("shell.menu")}>
                <Menu className="h-5 w-5" />
              </button>
            </div>
          </header>

          <nav className="fixed inset-x-0 bottom-0 z-20 hidden border-t bg-white shadow-lg md:block">
              ...
              {nav.map((n) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span className="font-medium">{t(n.tKey)}</span>
                  </Link>
                );
              })}
            </div>
          ...
          {drawerOpen && (
            <div className="fixed inset-0 z-40 md:hidden" onClick={() => setDrawerOpen(false)}>
              <div className="absolute inset-0 bg-black/40" />
              <div className="absolute bottom-0 left-0 right-0 rounded-t-2xl bg-white p-6 shadow-xl" onClick={(e) => e.stopPropagation()}>
                <div className="mb-4 flex items-center justify-between">
                  <span className="text-sm font-semibold text-slate-900">{manifest.bank.name}</span>
                  <button onClick={() => setDrawerOpen(false)} className="rounded p-1 text-slate-400 hover:bg-slate-100">
                    <X className="h-4 w-4" />
                ...
                <div className="mt-4 border-t pt-4">
                  <button
                    onClick={() => logoutMut.mutate()}
                    disabled={logoutMut.isPending}
                    className="flex w-full items-center gap-2 rounded-lg px-3 py-3 text-sm text-slate-600 hover:bg-slate-50"
                  >
                    <LogOut className="h-4 w-4" />
                    {logoutMut.isPending ? t("shell.logging_out") : t("shell.logout")}
                  </button>
          ...
          <main className="mx-auto w-full max-w-[1200px] flex-1 px-4 py-6 pt-16 pb-24 md:px-8 md:pb-6">{content}</main>
        </div>
      );
