    ...
    <div className="space-y-6">
      <BrandedCard manifest={bank.manifest}>
        <div className="flex items-center justify-between">
          <h1 className="text-xl font-semibold" style={{ color: primary }}>{t("nav.notifications")}</h1>
      ...
      <BrandedCard manifest={bank.manifest}>
        {items.length === 0 ? (
          <div className="rounded-md border border-dashed p-6 text-center text-sm opacity-70">{t("notifications.empty")}</div>
        ) : (
            ...
            {items.map((n) => (
              <li key={n.id} className="py-3 text-sm">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-xs opacity-60">{fmtDate(n.created_at, { dateStyle: "medium", timeStyle: "short" })}</span>
                </div>
